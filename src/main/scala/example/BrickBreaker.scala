...
case class BrickBreaker(bounds: Point, resetGame: () => Unit) extends Game {
  ...
  def draw(ctx: dom.CanvasRenderingContext2D) = {
    ...
    ctx.fillText(
      ...
      bounds.x - borderWidth / 2 - 40,
      4 * bounds.y / 5 - 10
    )
    for (i <- 1 to ballsLeft) {
      ctx.fillCircle(
        bounds.x - borderWidth / 2 - 40 + i * 15,
        4 * bounds.y / 5 + 10,
        5
      )
    }
  ...
  def update(keys: Set[Int]): Unit = {
    ...
    if (respawnCounter > 0) {
      ...
      paddlePos = paddlePos.copy(x =
        math.min(
          math.max(paddlePos.x, paddleDims.x / 2 + borderWidth),
          bounds.x - paddleDims.x / 2 - borderWidth
        )
      )
      ballPos = ballPos + ballVel
      if (ballPos.x + 5 > bounds.x - borderWidth)
        ...
        for (brick <- bricks) {
          ...
          for ((p1, p2, func) <- lines if !hit) {
            ...
            val perpDist =
              math.sqrt((ballPos - p1).lengthSquared - extent * extent)
            if (
              !hit && extent > 0 && extent < (p2 - p1).length && perpDist < 5
            ) {
