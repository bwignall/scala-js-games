...
case class Pong(bounds: Point, resetGame: () => Unit) extends Game {
  ...
  val leftPaddle =
    new Paddle(Point(40, bounds.y / 2), Point(5, 75), Point(0, 0), 0.5)
  val rightPaddle = new Paddle(
    Point(bounds.x - 40, bounds.y / 2),
    Point(5, 75),
    Point(0, 0),
    -0.5
  )
  ...
  def moveAI(): Unit = {
    ...
    if (
      math.abs(
        rightPaddle.pos.x - ballPos.x
      ) < ballVel.x * 45 + 25 && ballVel.x > 0
    ) {
      if (targetY > rightPaddle.pos.y + rightPaddle.dims.y / 2) {
        if (
          math.abs(
            targetY - rightPaddle.pos.y - (rightPaddle.dims.y / 2)
          ) > rightPaddle.dims.y / 2 - 10
        ) {
  ...
  def update(keys: Set[Int]): Unit = {
    ...
    for (p <- Seq(leftPaddle, rightPaddle)) {
      ...
      p.pos += p.direction
      p.pos = p.pos.copy(y =
        math.max(math.min(p.pos.y, bounds.y - p.dims.y / 2), p.dims.y / 2)
      )
